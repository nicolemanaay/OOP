...
def edit():
    ...
    conn = sqlite3.connect('C:/awoo/manaay.db')
    c = conn.cursor()
    ...
    c.execute("SELECT * FROM my_data WHERE oid=?", (record_id,))
    record = c.fetchone()
...
def submit():
    ...
    c.execute("INSERT INTO my_data VALUES (:f_name, :l_name, :age, :address, :email)",
              {
                'f_name': f_name.get(),
                'l_name': l_name.get(),
                'age': age.get(),
...
def delete():
    conn = sqlite3.connect('C:/awoo/manaay.db')
    c = conn.cursor()
    c.execute("DELETE FROM my_data WHERE oid=?", (delete_box.get(),))
    conn.commit()
